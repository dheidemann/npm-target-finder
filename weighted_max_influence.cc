...
class Graph {
public:
    // Map node ID (string in GEXF) to internal integer ID
    std::map<std::string, int> id_map;
    std::vector<std::string> reverse_id_map;
    
    // Adjacency list: adj[u] -> vector of edges
    std::vector<std::vector<Edge>> adj;
    
    // Node Weights (The attribute value)
    ...
    int get_internal_id(const std::string& gexf_id) {
        if (id_map.find(gexf_id) == id_map.end()) {
            ...
            adj.resize(new_id + 1);
            node_values.resize(new_id + 1, 0.0); // Default weight 0
            return new_id;
        }
        return id_map[gexf_id];
    }

    void add_edge(const std::string& src, const std::string& target, double prob) {
    ...
    int num_nodes() const {
        return reverse_id_map.size();
    }
};

// --- GEXF Parser (Dependency Free) ---
class GEXFParser {
public:
    static Graph parse(const std::string& filename, const std::string& target_attr_name) {
        Graph g;
        ...
        while (std::getline(file, line)) {
            // Very basic XML tag parsing
            size_t tag_start = line.find("<");
            if (tag_start == std::string::npos) continue;
            
            // 1. Find Attribute Definition ID
            // Example: <attribute id="0" title="revenue" type="float"/>
            if (line.find("<attribute") != std::string::npos) {
                ...
                if (title == target_attr_name) {
                    ...
                    std::cout << "Found Attribute ID for '" << title << "': " << target_attr_id << std::endl;
                }
            }

            // 2. Parse Nodes
            // Example: <node id="n1" label="Node 1">
            if (line.find("<node") != std::string::npos) {
                current_node_id = get_xml_attr(line, "id");
                g.get_internal_id(current_node_id); // Ensure node exists
                in_node = true;
                
                // Check if node tag is self-closing
                if (line.find("/>") != std::string::npos) in_node = false;
            }

            // 3. Parse Attribute Values (Weights)
            // Example: <attvalue for="0" value="100.5"/>
            if (in_node && line.find("<attvalue") != std::string::npos) {
                if (target_attr_id.empty()) continue; // Haven't found the attr def yet
            ...
            // 4. Parse Edges
            // Example: <edge source="n1" target="n2" weight="0.5"/>
            if (line.find("<edge") != std::string::npos) {
                ...
                // Default probability if weight not specified
                double prob = 0.1; 
                if (!w_str.empty()) {
                    try { prob = std::stod(w_str); } catch(...) {}
                }
        ...
        if (target_attr_id.empty()) {
            std::cerr << "Warning: Attribute '" << target_attr_name << "' not found in GEXF definitions." << std::endl;
        }
...
private:
    static std::string get_xml_attr(const std::string& line, const std::string& attr) {
...
// Returns SUM of node values (not just count)
double run_weighted_simulation(const Graph& g, const std::set<int>& seeds, std::mt19937& rng) {
    std::queue<int> q;
        ...
        for (const auto& edge : g.adj[u]) {
            ...
            if (active_set.find(v) == active_set.end()) {
                ...
                if (dist(rng) <= edge.probability) {
                    active_set.insert(v);
                    total_value += g.node_values[v]; // Add Value of newly activated node
                    q.push(v);
                }
            }
        }
...
double estimate_weighted_spread(const Graph& g, const std::set<int>& seeds) {
    ...
    for (int i = 0; i < MC_ROUNDS; ++i) {
        total_spread += run_weighted_simulation(g, seeds, rng);
    }
    return total_spread / MC_ROUNDS;
}

// --- CELF Optimization for Weighted Influence ---
struct NodeGain {
    int node_id;
...
std::set<int> celf_weighted_influence(const Graph& g, int k) {
    ...
    int n = g.num_nodes();
    std::cout << "Initializing CELF (calculating base weighted influence for " << n << " nodes)..." << std::endl;

    // 1. Initialization
    for (int i = 0; i < n; ++i) {
        // Optimization: If a node has 0 value and no outgoing edges, it has 0 gain. Skip it.
        if (g.node_values[i] == 0 && g.adj[i].empty()) continue;
    ...
    for (int iteration = 0; iteration < k; ++iteration) {
        ...
        while (!found_best && !pq.empty()) {
            ...
            if (seeds.find(top.node_id) != seeds.end()) continue;
...
int main(int argc, char* argv[]) {
    if (argc != 4) {
        std::cerr << "Usage: " << argv[0] << " <gexf_file> <k> <attribute_name>" << std::endl;
        return 1;
    }
    ...
    try {
        ...
        for (int s : seeds) std::cout << g.reverse_id_map[s] << " ";
        std::cout << std::endl;
